...
from .utils import (
    remove_messages_from_session,
    CIPHER_MESSAGE_SESSION_KEY,
    PLAINTEXT_MESSAGE_SESSION_KEY,
)
...
def index(request):
    ...
    if request.session:
        encode_text_form = Base64EncodeTextForm(
            initial={
                "message": request.session.get(PLAINTEXT_MESSAGE_SESSION_KEY, None)
            }
        )
        decode_form = Base64DecodeForm(
            initial={"message": request.session.get(CIPHER_MESSAGE_SESSION_KEY, None)}
        )
    else:
        encode_text_form = Base64EncodeTextForm()
        decode_form = Base64DecodeForm()
    ...
    if (
        FILE_NAME_SESSION_KEY in request.session
        and request.session[FILE_NAME_SESSION_KEY] is not None
    ):
...
def encode_file(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            request.session[CIPHER_MESSAGE_SESSION_KEY] = base64.encode(
                file_to_encode.read()
            )
        ...
        if form.is_valid():
            ...
            if is_file:
                ...
                file_name = os.path.join(
                    DECODE_DIRECTORY, str(datetime.utcnow().time())
                )
                ...
                try:
                    request.session[PLAINTEXT_MESSAGE_SESSION_KEY] = base64.decode_text(
                        message
                    )
                except UnicodeDecodeError:
                    request.session[PLAINTEXT_MESSAGE_SESSION_KEY] = str(
                        base64.decode_binary(message)
                    )
    return HttpResponseRedirect(reverse("gc_toolbox:base64"))
