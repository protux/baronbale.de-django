...
def index(request):
    if request.session and CIPHER_MESSAGE_SESSION_KEY in request.session:
        encrypt_form = CesarCipherEncryptForm(
            initial={"plain_message": request.session[PLAINTEXT_MESSAGE_SESSION_KEY]}
        )
    else:
        encrypt_form = CesarCipherEncryptForm()
    ...
    if request.session and PLAINTEXT_MESSAGE_SESSION_KEY in request.session:
        decrypt_form = CesarCipherDecryptForm(
            initial={"cipher_message": request.session[CIPHER_MESSAGE_SESSION_KEY]}
        )
    else:
        decrypt_form = CesarCipherDecryptForm()
...
def encrypt(request):
    if request.method == "POST":
        ...
        if encrypt_form.is_valid():
            ...
            request.session[CIPHER_MESSAGE_SESSION_KEY] = cesar_cipher.encrypt(
                key, message
            )
