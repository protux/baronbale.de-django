...
def encrypt(rot, text):
    ...
    for char in text:
        ...
        if index >= 0:
            char_index = __get_encrypted_index(index, key)
...
def __get_encrypted_index(index, key):
    index += key
    if index > 25:
        index -= 26
    return index
...
def decrypt(key, text):
    ...
    for c in text:
        try:
            ...
            if index >= 0:
                decrypted_text += ALPHABET[__get_decrypted_index(index, int(key))]
            else:
...
def __get_decrypted_index(index, key):
    index -= int(key)
