...
from baronbale_website.gc_toolbox.tools.banner_utils import extract_gpx_if_needed, join_banners
...
class BannerCollectorView(FormView):
    ...
    def form_valid(self, form):
        self.request.session[TIMER_SESSION_KEY] = datetime.datetime.utcnow().strftime(TIMER_FORMAT)
        ...
        try:
            ...
            self.put_banners_to_session(banners, horizontal_banners_per_row, vertical_banners_per_row)
    ...
    def handle_error(self):
        message_utils.add_error_message(self.request.session, _(
    ...
    def put_banners_to_session(self, banners, horizontal_banners_per_row, vertical_banners_per_row):
    ...
    def log_duration(self):
        start_time = datetime.datetime.strptime(self.request.session.pop(TIMER_SESSION_KEY), TIMER_FORMAT)
